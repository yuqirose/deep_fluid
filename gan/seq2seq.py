...
class EncoderRNN(nn.Module):
    def __init__(self, hidden_size, n_layers=1, args=None, params=None):
    ...
    def initHidden(self):
        h = torch.zeros(self.n_layers, self.args.batch_size, self.hidden_size)
        return h.to(self.params["device"])
...
class Seq2Seq(nn.Module):
    def __init__(self, args, params):
        ...
        self.encoder = EncoderRNN(self.args.h_dim, self.args.n_layers, args=args, params=params)

        self.use_attn = False
        if self.use_attn:
            self.decoder = AttnDecoderRNN(self.args.h_dim, args=args, params=params)
        else:
            self.decoder = DecoderRNN(self.args.h_dim, self.args.n_layers, args=args, params=params)
    ...
    def forward(self, x, y):
        ...
        decoder_input = torch.zeros(self.args.batch_size, self.args.d_dim, self.args.x_dim, self.args.y_dim)
        decoder_input = decoder_input.to(self.params["device"])
